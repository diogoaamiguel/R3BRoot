...
// New addition Silicon plate

Double_t det_xSiPos = 177 * rescaling; // x-size of helium box
Double_t det_ySiPos = 131 * rescaling; // y-size of helium box
Double_t det_zSiPos = 10.; // z-size of helium box

void create_rpc_small_Block_geo(const TString geoTag = "v2022.12")
{
    ...
    TString geoFileName = geoPath + "/geometry/tof_rpc_small_10cm_aluminium_Block_";
    ...
    TGeoMedium* pMed8 = gGeoMan->GetMedium("silicon");
    if (!pMed8)
    ...
    TGeoMedium* pMed9 = gGeoMan->GetMedium("lead");
    if (!pMed9)
        Fatal("Main", "Medium lead not found");
    ...
    double xbox = 178.4 * rescaling;
    double ybox = 158.4 * rescaling;
    double zbox = 30.;
    ...
    TGeoVolume* vol_al_Block = 
        gGeoManager->MakeBox("aluminium", pMed1, (det_xSiPos) / 2.0, (det_ySiPos) / 2.0, det_zSiPos / 2.0);
    vol_al_Block->SetLineColor(42);

    TGeoVolume* vol_si = 
        gGeoManager->MakeBox("silicon", pMed8, (det_xSiPos) / 2.0, (det_ySiPos) / 2.0, det_zSiPos / 2.0);
    vol_si->SetLineColor(42);

    TGeoVolume* vol_lead = 
        gGeoManager->MakeBox("lead", pMed9, (det_xSiPos) / 2.0, (det_ySiPos) / 2.0, det_zSiPos / 2.0);
    vol_lead->SetLineColor(42);

    // join everything

    // Silicon Plate
    Rpc_module->AddNode(vol_al_Block, 1, new TGeoTranslation(0, 0, -(det_zSiPos / 2.0)));
    // End Of Steel Plate

    // add FR4
    double z = 0;

    Rpc_module->AddNode(vol_al_F, 2, new TGeoTranslation(0, 0, (det_zAlPos / 2.0)));
    Rpc_module->AddNode(vol_FR4, 3, new TGeoTranslation(0, 0, det_zAlPos + (det_zFR4Pos / 2.0)));
    Rpc_module->AddNode(Cu_plane, 4, new TGeoTranslation(0, 0, det_zAlPos + (det_zFR4Pos + det_zStripPos / 2.0)));
